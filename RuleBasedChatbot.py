...
class SupportBot:
  ...
  exit_commands = ("quit", "pause", "exit", "goodbye", "bye", "later","Nothing")

  def __init__(self):
    self.matching_phrases = {'how_to_pay_bill': [r'.*how.*pay bills.*', r'.*how.*pay my bill.*'], r'pay_bill': [r'.*pay.*bill.*account.*number.*is (\d+)', r'.*need.*pay.*my.*bill.*account.*number.*is (\d+)']}
  ...
  def welcome(self):
    ...
    xd = input(f"Ok {self.name}, what can I help you with?\t ")
    will_help = xd.lower()
